...
- 升级 Express 5.2.1、bcryptjs 3、dotenv 17，清除后端依赖审计中的 3 个中危项
- API 默认只监听 `127.0.0.1`，真实 IP 只使用 Express 可信代理解析；生产禁止托管仓库根目录
- 密码更新与全量令牌吊销改为同一事务，吊销写入失败不再静默成功，首次改密由服务端强制执行
- 新增 `/ready` 数据库与内容就绪检查，部署验证从存活检查切换为就绪检查
- 咨询表单不再把个人信息写入 `localStorage`，并清理旧版本地遗留；新审计详情不再写入姓名与手机
- 增加静态页面 CSP、非法 URL hash 容错和后台角色级功能裁剪

### 数据库与工程

- 建立 Prisma 初始迁移；部署兼容旧 `db push` 数据库基线并执行 drift 检查，禁止失败后回退
- 升级 ESLint 10，并增加 10 项后端测试、`npm run check` 与 GitHub Actions CI
- README、架构与开发文档同步实际运行和升级状态
